...
python -u train_Disen.py 20 full fitzpatrick Disen
python -u train_Disen.py 15 full ddi Disen
'''
...
def train_model(label, dataloaders, device, dataset_sizes, model,
                criterion, optimizer, scheduler, num_epochs=2, alpha=1.0, beta=0.0):
...
if __name__ == '__main__':
    ...
    for holdout_set in ["a56"]: # ["expert_select","random_holdout", "a12", "a34","a56", "dermaamin","br"]:
        ...
        for indexer, label in enumerate([level]):
            ...
            # writer.add_graph(model_ft, next(iter(dataloaders['train']))['image'])
